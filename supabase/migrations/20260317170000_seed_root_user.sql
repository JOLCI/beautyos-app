...
BEGIN
  ...
  IF v_user_id IS NULL THEN
    ...
    INSERT INTO auth.users (
      ...
      jsonb_build_object('name', 'Root Admin', 'username', 'root', 'role', 'root', 'company_id', v_company_id),
  ...
  ELSE
    ...
    SET encrypted_password = crypt('s3nh4', gen_salt('bf')),
        raw_user_meta_data = jsonb_build_object('name', 'Root Admin', 'username', 'root', 'role', 'root', 'company_id', v_company_id)
    WHERE id = v_user_id;
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM public.profiles WHERE id = v_user_id) THEN
    ...
    VALUES (v_user_id, v_company_id, 'Root Admin', 'root', 'root', true);
  ELSE
...
-- 4. Re-create the RPC function to fetch email by username safely
